...
export default function CashVoucher() {
  ...
  const clearForm = () => { setItems([newItem()]); setPaidTo(''); };

  const buildData = () => ({
    title:'CASH PAYMENT VOUCHER' as const, vNo, date: new Date(date).toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'}),
    partyLabel:'Paid To', partyName: paidTo||'—', col3Label:'Account Head',
  ...
  if(view==='list') return (
    <div className="page-enter">
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text)', margin: 0 }}>Cash Payment Voucher</h2>
        <p style={{ color: 'var(--muted)', margin: '4px 0 0' }}>Record outgoing cash payments.</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
        <div className="stat-card"><div className="p-2.5 rounded-xl flex items-center justify-center shrink-0" style={{background:'rgba(190,24,93,.1)'}}><Wallet size={18} color="#be185d"/></div><div><div className="text-xs text-gray-400 font-medium">Total Vouchers</div><div className="text-xl font-bold text-gray-800 mt-0.5">{saved.length}</div></div></div>
        <div className="stat-card"><div className="p-2.5 rounded-xl flex items-center justify-center shrink-0" style={{background:'rgba(220,38,38,.1)'}}><Wallet size={18} color="#dc2626"/></div><div><div className="text-xs text-gray-400 font-medium">Total Paid</div><div className="text-xl font-bold text-gray-800 mt-0.5">₹{fmt(saved.reduce((s,v)=>s+v.total,0))}</div></div></div>
      </div>
      <div className="filter-bar mb-5">
        <button className="btn-brand flex items-center gap-2" onClick={()=>setView('form')}><Plus size={14}/> New Voucher</button>
      </div>
      <div className="card">
            ...
            <tbody>
              {saved.map((v,i)=>(
                <tr key={i}>
                  ...
                  <td style={{fontWeight:700,color:'var(--red)'}}>₹{fmt(v.total)}</td>
  ...
  return (
    <div className="page-enter" style={{paddingBottom:80}}>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-lg font-bold text-gray-800">New Cash Payment Voucher</h2>
          <p className="text-xs text-gray-400 mt-0.5">Record an outgoing cash payment</p>
        </div>
        <button className="btn-outline flex items-center gap-2 text-xs" onClick={()=>setView('list')}><FileText size={13}/> Voucher List</button>
      </div>
      ...
      <div className="card mb-4 p-5">
        <div className="grid grid-cols-3 gap-4">
          ...
          <div><label className="form-label">PAID TO <span className="text-red-500">*</span></label><input className="inp" value={paidTo} onChange={e=>setPaidTo(e.target.value)}/></div>
        </div>
      </div>

      <div className="card mb-4">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100"><span className="font-semibold text-sm text-gray-700">Voucher Rows</span></div>
        <div className="overflow-x-auto">
          <table style={{width:'100%', borderCollapse:'collapse', minWidth:700}}>
            <thead><tr style={{background:'#fdf2f8'}}>{['Ref','Description','Account Head','Amount (₹)',''].map(h=>(
            ...
            <tbody>
              {items.map(it=>(
                ...
                <tr style={{borderBottom:'1px solid #f3f4f6'}}>
                  ...
                  <td style={{padding:'6px 8px', minWidth:160}}><input className="inp py-1 text-xs w-full" value={it.col3} onChange={e=>updateItem(it.id,'col3',e.target.value)}/></td>
      ...
      <div style={{position:'fixed', bottom:0, left:240, right:0, background:'#fff', borderTop:'1px solid #e5e7eb', padding:'12px 24px', display:'flex', alignItems:'center', gap:12, zIndex:50, boxShadow:'0 -2px 12px rgba(0,0,0,0.06)'}}>
        ...
        <button onClick={saveVoucher} style={{background:'#d97706',color:'#fff',border:'none',borderRadius:8,padding:'8px 20px',fontSize:13,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',gap:8}}><FileText size={14}/> Save Voucher</button>
      </div>
    </div>
  );
}
