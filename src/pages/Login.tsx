...
export default function Login() {
  ...
  return (
    <div className="login-wrap" style={{ position:"fixed", inset:0, background:"#fdf2f8", display:"flex", alignItems:"center", justifyContent:"center", padding:20, overflowY:"auto" }}>
      <style>{`
        .login-card { display:flex; width:100%; max-width:920px; border-radius:20px; overflow:hidden; box-shadow:0 24px 80px rgba(0,0,0,.18); min-height:560px; }
        .login-left { flex:1; padding:40px 36px; display:flex; flex-direction:column; position:relative; overflow:hidden; }
        .login-right { width:400px; flex-shrink:0; padding:44px 40px; display:flex; flex-direction:column; justify-content:center; position:relative; overflow:hidden; }
        @media (max-width: 760px) {
          .login-wrap { padding:0; align-items:flex-start; }
          .login-card { flex-direction:column; max-width:100%; min-height:100vh; border-radius:0; box-shadow:none; }
          .login-left { padding:28px 20px 24px; flex:none; }
          .login-right { width:100%; padding:28px 20px 40px; }
          .login-heading { font-size:19px !important; }
          .login-stats { grid-template-columns:1fr 1fr !important; }
        }
        @media (max-width: 400px) {
          .login-stats { gap:8px !important; }
        }
      `}</style>
      <div className="login-card">

        {/* ── LEFT PANEL ── */}
        <div className="login-left" style={{ background:"#3f1d33" }}>
          ...
          <div className="login-heading" style={{ fontSize:22, fontWeight:800, color:"#fff", lineHeight:1.3, marginBottom:10, position:"relative" }}>Flower Business Management Dashboard</div>
          <div style={{ fontSize:12.5, color:"#d8b4cf", lineHeight:1.7, marginBottom:20, position:"relative" }}>
            Manage billing, inventory, quotations, leads &amp; users for weddings, events and decoration orders — Wilson Garden, Bangalore.
          </div>

          {/* Stats grid */}
          <div className="login-stats" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:20, position:"relative" }}>
            {[["1985","Since"],["500+","Events"],["24/7","Support"],["GST","Registered"]].map(([val,lbl])=>(
              <div key={lbl} style={{ background:"rgba(255,255,255,.06)", border:"1px solid rgba(255,255,255,.08)", borderRadius:12, padding:"14px 16px", minWidth:0 }}>
          ...
          {/* Footer */}
          <div style={{ marginTop:"auto", paddingTop:16, borderTop:"1px solid rgba(255,255,255,.08)", position:"relative" }}>
        ...
        <div className="login-right" style={{ background:"#fff" }}>
          ...
          <div style={{ position:"relative", marginBottom:16 }}>
            <input
              ...
              style={{ width:"100%", padding:"11px 14px", border:"1.5px solid #f3d9e8", borderRadius:10, fontSize:16, fontFamily:"inherit", color:"#3f1d33", outline:"none", background:"#fdf2f8", boxSizing:"border-box" }}
            />
          </div>
          ...
          <div style={{ position:"relative", marginBottom:20 }}>
            <input
              ...
              style={{ width:"100%", padding:"11px 40px 11px 14px", border:"1.5px solid #f3d9e8", borderRadius:10, fontSize:16, fontFamily:"inherit", color:"#3f1d33", outline:"none", background:"#fdf2f8", boxSizing:"border-box" }}
            />
          ...
          {DEMO.map(d=>(
            <div key={d.role} onClick={()=>{setUser(d.user);setPass("bloom@2026");}}
              style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"9px 12px", borderRadius:9, cursor:"pointer", border:"1px solid transparent", marginBottom:4, transition:"all .15s", gap:8, flexWrap:"wrap" }}
            ...
            >
              ...
              <span style={{ fontSize:12, color:"#8a6b83", wordBreak:"break-all" }}>{d.user}</span>
            </div>
          ))}
        </div>
